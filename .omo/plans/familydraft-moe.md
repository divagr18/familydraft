...
- [ ] 22. Phase-1 campaign + ablation matrix (MILESTONE 3) [PARTIAL - infra SHIPPED: pre-registered verdict_protocol.yaml + baseline_report.schema.json + FLOP accounting (equal-FLOP dense baseline) + unified harness run_baselines.py (--all runs 6 systems x 4 task classes, verified 24/24) + phase1.csv + m3_verdict.py + m3_order_check.py (PASS) + determinism self-check (check_determinism.py, PASS/FAIL-mutation verified) + row reproducibility (run_phase1.py --row <hash>, spot-checked <3%) + 12-config pre-registered ablation matrix (configs/ablations/, all verified runnable) + EAGLE-3 SpecForge pod pipeline (setup/eval/resume) + pod orchestrator run_phase1_8b.sh; local 0.6B campaign RUN -> honest FAIL (DAG < vanilla and < equal-FLOP dense at 0.6B) + full 12-config x 4-task ablation matrix RUN at 0.6B (48 rows aggregated into phase1.csv, distinct abl_*.json rows); REMAINS pod-deferred: 8B campaign + EAGLE-3 training via SpecForge + MoE probe rows]
...
- [ ] 23. Phase-1 verdict report + concept-note reference corrections [report exists but contains overclaims; corrected via Corrigendum; re-open]
