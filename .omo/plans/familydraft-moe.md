...
- [ ] 22. Phase-1 campaign + ablation matrix (MILESTONE 3) [PARTIAL - infra shipped: pre-registered verdict_protocol.yaml + baseline schema + FLOP accounting + unified harness (6 systems x 4 task classes) + phase1.csv + m3_verdict.py + m3_order_check.py (PASS); local 0.6B campaign run -> honest FAIL (DAG < vanilla and < equal-FLOP dense at 0.6B); EAGLE-3 pod pipeline (setup/eval/resume) written but 8B training pod-deferred; full 8B campaign + 12-ablation matrix remain]
